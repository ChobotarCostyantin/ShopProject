    ...
    public class OrderService : IOrderService
    {
        ...
        public async Task<Result<PaginationResult<OrderDto>>> GetOrdersAsync(GetOrdersRequest request, CancellationToken cancellationToken)
        {
            try
            {
                ...
                var orders = await _unitOfWork.OrderRepository.GetOrdersAsync(request.PageSize, request.PageNumber, cancellationToken);
                var totalCount = await _unitOfWork.OrderRepository.CountAllOrdersAsync(cancellationToken);
